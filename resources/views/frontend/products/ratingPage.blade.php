...
<?php
	...
	Carbon::setLocale('vi');
?>
<h5>{{ $rating_comment_count }} Đánh giá</h5>
<hr>
<?php if ($rating_comments->count() > 0): ?>
    ...
    <div class="media p-0">
	    <div class="media-body" style="padding-bottom: 15px">
	      	...
	      	<?php
	      		...
	      		for ($i=0; $i < $rating; $i++) { 
	      			echo "<span class='fas fa-star fa-sm text-warning'></span>";
	      		}
	      	 ?>
...
{{ $rating_comments->links('vendor/pagination/bootstrap-4-rating') }}
	...
	$(document).ready(function() {
		...
		<?php foreach ($rating_comments as $comment): ?>
	        ...
	        $(document).on('submit', '#reply_comment_form{{ $comment->id }}', function(event) {
	            ...
	            // load Rating Reply Comment
	            function loadRatingReplyComment() {
	                $.ajax({
			            url: '{{ route('rating.replycomment.loadpage', $comment->id)}}',
			        })
			        .done(function(data) {
			            // console.log(data);
